...
def mysql_visibility_condition(
    ...
    if scope is None:
        return (
            ...
            " AND (visible_repo.active_sync_run_id="
...
def chroma_visibility_filter(
    ...
    *extra_conditions: dict,
    ...
    if scope.active_run_ids:
        visible.append({"repo_sync_run_id": {"$in": list(scope.active_run_ids)}})
    if scope.legacy_repo_ids:
        visible.append(
            {
                "$and": [
    ...
    visible_condition = visible[0] if len(visible) == 1 else {"$or": visible}
    return _and_conditions(
        ({"project_id": scope.project_id}, visible_condition, *extra_conditions)
    )
