...
def _suggestion_or_404(cursor, project_id: int, suggestion_id: int) -> dict:
    ...
    cursor.execute(
        ...
        " JOIN active_memory m"
        "  ON m.id = s.memory_id AND m.project_id = s.project_id"
...
def _apply_accepted_effect(cursor, project_id: int, row: dict) -> None:
    ...
    if kind == "supersede":
        ...
        cursor.execute(
            ...
            f" AND {published_sql}",
            [superseding_id, row["memory_id"], project_id, *published_params],
        )
    ...
    if kind == "complete_action":
        ...
        if not row.get("memory_completed_at"):
            ...
            cursor.execute(
                ...
                f" AND {published_sql}",
                [row["memory_id"], project_id, *published_params],
            )
            if cursor.rowcount == 0:
                raise HTTPException(
                    status_code=409,
        ...
        with conn.cursor() as cursor:
            ...
            if status == "accepted":
                ...
                if row["kind"] == "supersede":
                    # The accepted edge hides only its old target. Close every
                    # pending edge that would become unreachable because it
                    # references that inactive memory, while preserving valid
                    # follow-up edges involving the still-active replacement.
                    cursor.execute(
                        ...
                        "evidence,'$.superseding_memory_id')) AS UNSIGNED)"
                        " =%s)",
                        (
                            resolved_by,
                            project_id,
