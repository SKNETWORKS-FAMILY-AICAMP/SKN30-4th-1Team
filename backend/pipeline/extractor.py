...
# 모델이 해석하는 설명·규칙은 한국어로 통일한다. 다만 category 값
# (decision/action/issue/risk)과 JSON 필드명은 DB·API가 그대로 쓰는 기계 계약이므로
# 영문을 유지한다.
...
- 명시적으로 확인되는 항목만 하나의 객체로 추출하고, 불명확한 내용은 만들지 않습니다.
...
- action: 수행해야 할 구체 작업. 예: "홍길동님이 API 명세서를 작성하겠습니다"
...
# source_kind별 추가 지침. 공통 규칙 앞에 붙어 우선 적용된다.
_REPO_PROMPTS = {
    "repo_readme": """
